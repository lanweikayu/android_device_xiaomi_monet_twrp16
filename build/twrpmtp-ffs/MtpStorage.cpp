...
MtpObjectHandleList* MtpStorage::getObjectList(__attribute__((unused)) MtpStorageID storageID, MtpObjectHandle parent) {
		ALOGE("DBG MtpStorage::getObjectList storageID=%u parent=%u mtpmap_size=%u", storageID, parent, (unsigned)mtpmap.size());
		...
		ALOGE("DBG MtpStorage::getObjectList returning list size=%u", (unsigned)list->size());
...
int MtpStorage::getObjectPropertyList(MtpObjectHandle handle, uint32_t format, uint32_t property, int groupCode, __attribute__((unused)) int depth, MtpDataPacket& packet) {
		...
		packet.putUInt32(results.size());
			...
			for (size_t i = 0; i < results.size(); ++i) {
					...
					// MTP date properties (DATE_MODIFIED/DATE_ADDED) are strings
					// on the wire even though they are stored internally as
					// uint64.  Override the datatype to STR so the host parses
					// the value as a string; otherwise libmtp/gvfs reads the
					// raw integer (or the string below) with the wrong type and
					// crashes when enumerating directories.
					if (p.property == MTP_PROPERTY_DATE_MODIFIED ||
							p.property == MTP_PROPERTY_DATE_ADDED) {
					...
					switch (p.datatype) {
						...
						case MTP_TYPE_UINT64:
								...
								// MTP date properties are strings on the wire but are
								// stored internally as uint64 (see IMtpDatabase::
								// getObjectPropertyValue for the same special case).
								// Without this, hosts (libmtp/gvfs) that hard-code
								// DATE_MODIFIED/DATE_ADDED as STRING will strlen() the
								// raw integer value and crash.
								if (p.property == MTP_PROPERTY_DATE_MODIFIED ||
										p.property == MTP_PROPERTY_DATE_ADDED) {
									char date[20];
									formatDateTime(p.intvalue, date, sizeof(date));
