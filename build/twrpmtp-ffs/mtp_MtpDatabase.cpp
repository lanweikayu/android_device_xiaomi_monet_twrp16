...
MtpObjectHandleList* IMtpDatabase::getObjectList(MtpStorageID storageID,
  ...
  ALOGE("DBG getObjectList storageID=%u parent=%u", storageID, parent);
  MtpObjectHandleList* list = storagemap[storageID]->getObjectList(storageID, parent);
  ALOGE("DBG getObjectList returned %p", list);
  if (list) ALOGE("DBG getObjectList size=%u", (unsigned)list->size());
  return list;
}
...
int IMtpDatabase::getNumObjects(MtpStorageID storageID,
								__attribute__((unused)) MtpObjectFormat format,
								MtpObjectHandle parent) {
  ALOGE("DBG getNumObjects storageID=%u parent=%u", storageID, parent);
  MtpObjectHandleList* list = storagemap[storageID]->getObjectList(storageID, parent);
  int size = list->size();
  ALOGE("DBG getNumObjects size=%d", size);
  delete list;
  return size;
}
