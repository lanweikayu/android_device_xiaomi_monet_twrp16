...
RECOVERY_BINARY_SOURCE_FILES += \
    ...
    $(TARGET_OUT_EXECUTABLES)/avb_disable

# avb_disable links against libcrypto; make sure the shared lib is present
# in the recovery ramdisk (TWRP already ships it for avbctl).
TW_RECOVERY_ADDITIONAL_RELINK_LIBRARY_FILES += \
...
# Theme is generated by soong's copyTheme into $(TARGET_RECOVERY_ROOT_OUT)/twres;
# patch it in place after the ramdisk is assembled (instead of shipping a
# pre-built copy): point "Disable AVB2.0" at avb_disable (re-signs all five
# vbmeta partitions), drop the broken auto-disable-avb2 checkbox, and remove
# the "Install TWRP App" entry.  bootable/recovery source is untouched.
BOARD_RECOVERY_IMAGE_PREPARE += \
    python3 $(DEVICE_PATH)/build/theme_patch.py $(TARGET_RECOVERY_ROOT_OUT)/twres/portrait.xml;
